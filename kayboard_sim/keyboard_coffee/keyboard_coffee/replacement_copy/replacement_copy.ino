...
// static uint8_t out_pins[OUT_PINS_CNT] = { 10,11,12,14,15}; standard


#define SCANNING_PINS_CNT 4
static uint8_t scanning_pins[SCANNING_PINS_CNT] = { 7,8,9,10};
// static uint8_t scanning_pins[SCANNING_PINS_CNT] = { 16,17,18,19}; standard
...
void loop() {
  if (Serial.available() > 0) {
    ...
    if (ch == 'Y') {
      ...
      keyboard_connection();
      for (uint8_t i = 0; i < OUT_PINS_CNT; i++) pinMode(out_pins[i], INPUT_PULLUP);
      for (uint8_t i = 0; i < SCANNING_PINS_CNT; i++) pinMode(scanning_pins[i], INPUT_PULLUP);
      c_pos = { -1, -1 };
    } else {
